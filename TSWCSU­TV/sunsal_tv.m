function [U,lres,sre] = sunsal_tv(M,Y,varargin)
...
while (i <= AL_iters) && (sum(abs(res)) > tol1)
    ...
    for j=1:(n_reg+1)
        ...
        % l1 norm  (V4)
        if  reg(j) == 4
            V{j} = vector_hard_row(Uj-D{j},2*lambda_l1/mu);
        end
        % TV  (V5 and V6)
        if  reg(j) == 5
...
%         st = [];
        for j=1:(n_reg+1)
            if  reg(j) == 1
                res(j) = norm(M*Uj-V{j},'fro');
               % st = strcat(st,sprintf(' res(%i) = %2.6f',reg(j),res(j) ));
            else
                res(j) = norm(Uj-V{j},'fro');
               % st = strcat(st,sprintf('  res(%i) = %2.6f',reg(j),res(j) ));
            end
        end
        lres(i)=sum(abs(res));
...
%         end
    ...
    % compute RMSE
    if true_x
        sre(i)=20*log10(norm(XT,'fro')/norm(Uj-XT,'fro'));  %norm(Uj-XT,'fro');
%         if  strcmp(verbose,'yes')
