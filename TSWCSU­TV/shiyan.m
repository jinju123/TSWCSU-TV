...
SRE_s = 20*log10(norm(XT,'fro')/norm(X_hat_s-XT,'fro')); 
...
SRE_l21 = 20*log10(norm(XT,'fro')/norm(X_hat_l21-XT,'fro')); 
...
lambda = 5e-1;
lambda_TV = 1e-2;
[X_hat_ftv,res,rmse_ni] = TSWCSU_TV(A,Y,'MU',0.05,'POSITIVITY','yes','ADDONE','no', ...
                               'LAMBDA_1',lambda,'LAMBDA_TV', lambda_TV, 'TV_TYPE','niso',...
                               'IM_SIZE',[75,75],'AL_ITERS',300, 'TRUE_X', XT,  'VERBOSE','yes');
                          
SRE_ftv = 20*log10(norm(XT,'fro')/norm(X_hat_ftv-XT,'fro')); 
...
fprintf('SRE-s = %2.3f\nSRE-l21 = %2.3f\nSRE_tv-ni = %2.3f\nSRE-ftv = %2.3f\n\n', ...
            SRE_s, SRE_l21,SRE_tv_ni, SRE_ftv)
       
        
% endmember no. 1 (cls)
X_hat_s_im = reshape(X_hat_s', nl,nc,n);       
figure(2)
imagesc(X_hat_s_im(:,:,supp(3)))
axis square
title('SUnSAL - Frational abundance of endmember 3')


% endmember no. 1 (l2-l1)
X_hat_l21_im = reshape(X_hat_l21', nl,nc,n);       
figure(3)
imagesc(X_hat_l21_im(:,:,supp(3)))
axis square
title('CLSUnSAL - Frational abundance of endmember 3')


% endmember no. 1 (tv_ni)
X_hat_tv_ni_im = reshape(X_hat_tv_ni', nl,nc,n);       
figure(4)
imagesc(X_hat_tv_ni_im(:,:,supp(4)))
axis square
title('SUnSAL-TV (NISO) - Frational abundance of endmember 5')


% endmember no. 1 (ftv_ni)
X_hat_ftv_im = reshape(X_hat_ftv', nl,nc,n);       
figure(5)
imagesc(X_hat_ftv_im(:,:,supp(4)))
axis square
title('SUnSAL-FTV - Frational abundance of endmember 5')
...
figure(7)
imagesc(X_hat_s(:,1:100))
axis  square
title('SUNSAL')

figure(8)
imagesc(X_hat_l21(:,1:100))
axis   square
title('CLSUnSAL')
...
figure(10)
imagesc(X_hat_ftv(:,1:100))
axis  square
title('SUnSAL-FTV-ISO')
