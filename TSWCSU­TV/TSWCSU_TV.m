function [Ug,rmseftv] = TSWCSU_TV(M,Y,alpha,varargin)
...
%                           Default 0.001;
%  'alpha' - (double):   the weight for two-step cofficient alpha
...
%     Ug=U+((gg0-1)/gg)*(U-Ugg);
%     
 Ug=(alpha)*U+(1-alpha)*Ugg;
    
    % Compute the Mourau proximity operators
    for j=1:(n_reg+1)
        %  data term (V1)
        if  reg(j) == 1
            V{j} = (1/(1+mu)*(Y+mu*(M*Ug-D{j})));
        end
        %  positivity   (V2)
        if  reg(j) == 2
            V{j} = max(Ug-D{j},0);
        ...
        % l1 norm  (V4)
        if  reg(j) == 4
%             V{j} = soft(Uj-D{j},lambda_l1/mu );
 V{j} =  vector_soft_row(Ug-D{j},lambda_l1/mu./(sqrt(sum(abs(Ug-D{j}).^2,2))+ eps )+eps);
%               V{j} = soft(Uj-D{j},lambda_l1/mu./( abs(U-D{j}) + eps));
%                V{j} =  vector_soft_row(Ug-D{j},lambda_l1/mu);
        end
        % TV  (V5 and V6)
        if  reg(j) == 5
    ...
    if mod(i,10) == 1
        ...
        for j=1:(n_reg+1)
            if  reg(j) == 1
                res(j) = norm(M*Ug-V{j},'fro');
%                 st = strcat(st,sprintf(' res(%i) = %2.6f',reg(j),res(j) ));
            else
                res(j) = norm(Ug-V{j},'fro');
%                 st = strcat(st,sprintf('  res(%i) = %2.6f',reg(j),res(j) ));
            end
        end
...
%         end
    ...
    % compute RMSE
     if true_x
         rmseftv(i)= norm(Ug-XT,'fro')/(norm(XT,'fro'));
        %SRE_ftv(i)= 20*log10(norm(XT,'fro')/norm(Ug-XT,'fro'));  
%          if  strcmp(verbose,'yes')
%             fprintf(strcat(sprintf('iter = %i -SRE = %2.3f',i, SRE_ftv(i)),'\n'));
%         end
        
%         if  strcmp(verbose,'yes')
%             fprintf(strcat(sprintf('iter = %i - ||Xhat - X|| = %2.3f',i, rmseftv(i)),'\n'));
%         end
